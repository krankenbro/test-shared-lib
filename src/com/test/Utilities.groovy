...
class Utilities {
    ...
    def static runUnitTest(context, traits, paths, resultsFileName)
    {
        ...
        if(isNetCore(context.projectType)){
            context.bat "\"${context.env.OPENCOVER}\\opencover.console.exe\" -returntargetcode -searchdirs:\"${pdbDirs}\" -register:user -filter:\"+[Virto*]* -[xunit*]*\" -output:\"${coverageFolder}\\VisualStudio.Unit.coveragexml\" -target:\"${context.env.DOTNET_PATH}\\dotnet.exe\" -targetargs:\"vstest ${paths} /TestCaseFilter:(${traits}) /Enablecodecoverage\""
        }
        else{
            context.bat "\"${context.env.OPENCOVER}\\opencover.console.exe\" -returntargetcode -searchdirs:\"${pdbDirs}\" -register:user -filter:\"+[*]* -[Moq]* -[xunit*]* -[Common.*]*\" -output:\"${coverageFolder}\\VisualStudio.Unit.coveragexml\" -target:\"${context.env.DOTNET_PATH}\\dotnet.exe\" -targetargs:\"vstest ${paths} /TestCaseFilter:(${traits}) /Enablecodecoverage\""
        }
